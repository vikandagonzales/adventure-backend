...
const create = (req, res, next) => {
  if (!req.body.name || !req.body.limit) return next({status: 400, message: 'Missing information'});
...
const update = (req, res, next) => {
  if (!req.body.name && !req.body.limit) return next({status: 400, message: 'No changes to make'});
